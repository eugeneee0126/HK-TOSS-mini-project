# kakao_search_integrated.py
# Description: 카카오맵에서 특정 키워드(예: 충정로역 맛집) 검색 결과를 크롤링하는 스크립트
#              - 가게 기본정보, 영업시간, 시설정보, 해시태그, 카테고리, 대표사진, 메뉴를 수집
#              - 최대 50개 가게까지 크롤링
#              - 3페이지까지는 첫번째 로직, 그 이후는 두번째 로직을 사용
# Author: 통합버전
# Date: 2025.04.29
...
def main():
    with sync_playwright() as p:
        ...
        while crawled_stores_count < max_store_id:
            ...
            if current_page < 3:
                ...
                try:
                    ...
                    page.wait_for_selector(next_page_selector, timeout=5000)
                    page.click(next_page_selector)
                    print(f"➡️ 페이지 {next_page_num} 클릭 (두번째 로직)")
                    page.wait_for_selector('ul#info\\.search\\.place\\.list', timeout=10000)
                    time.sleep(2)
                except Exception as e:
        ...
        # 저장 파일명
        stores_csv = f"{search_keyword.replace(' ', '_')}_stores.csv"
        menus_csv = f"{search_keyword.replace(' ', '_')}_menus.csv"
